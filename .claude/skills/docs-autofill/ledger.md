...
Outcomes: `prepared` (work committed, PR not yet created) · `opened` · `blocked` (3 rounds,
persona still stuck) · `skipped-no-key` · `skipped-code-defect` (the page could not be
written honestly around a code/docs inconsistency — see the Slack message for that run).
