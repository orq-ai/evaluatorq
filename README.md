...
Every red team and simulation run is saved locally. `eq dashboard` serves them all — filter findings, read transcripts, compare runs, export HTML/CSV/JSON, and use **Find** to turn a natural-language question into JEV judgments over recent Orq traces. See the [Trace finder guide](https://orq-ai.github.io/evaluatorq/trace-finder/) for the dashboard and `eq find` workflows.
