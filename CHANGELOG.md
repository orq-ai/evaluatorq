...
- `evaluatorq()` now returns results to library callers when an evaluator reports `pass_=False` (previously it exited the process with status 1). CLI workflows retain explicit failure handling and can opt into process exit.
