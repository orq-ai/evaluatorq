...
- Every rendered dollar figure is now qualified with its coverage and provenance. Costs on the red-team and simulation reports, the dashboard KPI band and the dashboard run tables carry the `cost_coverage` clause (`' (3 of 10 calls)'`, `' (estimated)'`, `' (3 of 10 calls, partly estimated)'`); the dashboard run-table cell renders it as a leading `~` with the full wording in its tooltip. A figure with **no** qualifier now means exactly one thing: every call behind it was billed by the provider. On the dashboard, which distinguishes `unknown_calls` from `priced_calls`, aggregates that mix in reports written before coverage was tracked say so explicitly (`'N calls of unknown coverage'`) rather than rendering an empty clause. The red-team and simulation Markdown/HTML reports do not carry that distinction: `common.reports.md_helpers.cost_coverage` returns `''` whenever `priced_calls` is `0`, so a report predating coverage tracking still renders a bare dollar figure there — a qualifier's absence means "known-covered" on the dashboard, but is ambiguous with "coverage unknown" on those two report surfaces. LLM spans gained a `gen_ai.usage.cost_source` attribute alongside `gen_ai.usage.cost` — see [Tracing](https://orq-ai.github.io/evaluatorq/tracing/).
- A cost total that was priced client-side now says so. `Usage` gained `estimated_calls` (how many of `priced_calls` came from `common.model_catalogue.price_usage` — token counts × the catalogue's per-1k rate — rather than a provider-reported cost) and a `cost_source` property (`'provider' | 'catalogue' | 'mixed' | None`). Report labels built with `common.reports.md_helpers.cost_coverage` now read `' (estimated)'` or `' (3 of 10 calls, partly estimated)'` instead of a bare call count, across the red-team, simulation and dashboard cost surfaces. The estimate itself is unchanged and remains approximate: it applies a flat input rate, while cached-read and cache-write tokens bill at a discount/premium the catalogue does not publish.
- `RedTeamReport.token_usage_summary` removed. The attribute was public but always `None` — nothing ever populated it, and `RedTeamReport` tolerates the extra key on deserialization, so no caller observes a behavior change from its absence. Read `RedTeamReport.summary.token_usage_total` for the run's usage instead.

### Breaking Changes

- `llm_jury(assignment='all')` now returns `raw_output={'endpoint': ...}` where it previously returned `None`. The value is `'chat'`, `'responses'`, or `'mixed'` when a panel used both, and records which endpoint actually served each verdict — it separates "the model is not in the Responses catalogue" from "Responses 400'd and we fell back to chat". The docs previously stated that `assignment='all'` carries no `raw_output`; code reading `result.raw_output is None` to detect the all-assignment mode must switch to checking the assignment itself. Single-judge and panel/jury paths stamp the same key.
- `Usage.with_calls()` removed (`Usage` is re-exported as `TokenUsage`). It returned a copy with `calls` set and `priced_calls` derived from whether a cost was present — which is exactly what `Usage.extract(usage, calls=N)` already does at parse time, so every in-repo caller was patching a count back on one line after the parse that should have counted it. Both parsers now count their own call and the method is gone; external code that hand-builds a `Usage` for a custom `AgentTarget` and calls `.with_calls(n)` gets an `AttributeError`. Replace `Usage.extract(raw, calls=0).with_calls(n)` with `Usage.extract(raw, calls=n)`, or pass `calls=` to the constructor directly.
- `AgentResponse.from_openresponses` now parses one response object as `calls=1` (previously `calls=0`). One response object is one API call, matching `Usage.from_completion` on the chat side; `priced_calls` follows from whether the payload carried a cost. **External callers that added their own `+1` afterward now double count** — and an inflated `calls` alongside an inflated `priced_calls` makes a partial cost read as fully covered, silently. Drop the manual increment. An aggregated usage dump that carries its own `calls` still overrides this.
- `MultiTurnOrchestrator.generate_single_prompt` removed. It was a public method with no caller anywhere in `src/`, `tests/`, `docs/`, or `examples/`, and `MultiTurnOrchestrator` is not re-exported from `evaluatorq.redteam` — so this is unlikely to affect anyone, but it is a real removal from a public class's surface.
