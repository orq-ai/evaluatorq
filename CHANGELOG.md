...
- Token-usage tables in the red-team and simulation reports now report **cached input tokens as a share of input** (`75 (75% of input)`) rather than a bare count. A raw cached figure says nothing about whether caching worked without the denominator it was cached against. Red-team reports gained the row outright — they carried only cache *writes*, which the Orq router reports as permanently 0, so the one measurable cache number was missing from the surface an operator reads. The row is omitted entirely when nothing was cached, so `0 (0% of input)` never reads as a measured result.
