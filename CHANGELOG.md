...
- A simulation whose **target** fails mid-run now keeps the criteria audit collected before the failure. The error result carries the folded `rules_broken`, `criteria_results`, `criteria_meta` and `criteria_verified` instead of `rules_broken=[]` and no metadata, so a `must_not_happen` violation the judge confirmed on turn 2 survives the target dying on turn 4 — it previously vanished from the result, the report and `find_triggers`. Such a run is still scored `0.0` by `criteria_met`, because it terminated by error.
