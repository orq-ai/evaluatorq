...
- A simulation whose **target** fails mid-run now keeps the criteria audit collected before the failure. The error result carries the folded `rules_broken`, `criteria_results`, `criteria_meta` and `criteria_verified` instead of `rules_broken=[]` and no metadata, so a `must_not_happen` violation the judge confirmed on turn 2 survives the target dying on turn 4 — it previously vanished from the result and the report. (It never reached `find_triggers`: that helper returns `[]` for any errored result before it looks at criteria, and still does.) The fold is guarded on the audit having happened: a target that dies before the judge audits **anything** reports its criteria as `unknown` (`criteria_verified=False`, `audited: False`, row state `unknown`), never as failed — folding the not-observed defaults there would have invented a `must_happen` failure the judge never made. Such a run is still scored `0.0` by `criteria_met`, because it terminated by error.
