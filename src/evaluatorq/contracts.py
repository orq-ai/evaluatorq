...
class Usage(BaseModel):
    ...
    def extract(cls, usage: Any, *, calls: int = 1) -> Usage | None:
        ...
        if usage is None:
            return None
        inp = _usage_first_int(usage, ('input_tokens', 'prompt_tokens', 'promptTokens', 'prompt', 'input'))
        out = _usage_first_int(
            usage, ('output_tokens', 'completion_tokens', 'completionTokens', 'completion', 'output')
        )
        ...
        # Codex CLI is flat too (`cached_input_tokens`); OpenCode nests `cache.read`.
        cached = (
            _usage_detail_int(
                usage,
            ...
            or _usage_detail_int(usage, ('cache',), ('read',))
...
class JuryRepetition(BaseModel):
    ...
    raw_output: dict[str, Any] | None = Field(
        default=None,
        description='Validated structured judge output for this pass; None when the judge exposes no structured detail.',
    )
