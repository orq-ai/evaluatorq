...
async def _do_call(
    *,
    ...
    async with with_llm_span(
        ...
        trace_id = telemetry.get('trace_id') if isinstance(telemetry, dict) else getattr(telemetry, 'trace_id', None)
        span_id = telemetry.get('span_id') if isinstance(telemetry, dict) else getattr(telemetry, 'span_id', None)
    ...
    return _normalise_response(self, response, trace_id=trace_id, span_id=span_id)
        ...
        *,
        ...
        try:
            ...
            return await with_retry(
                lambda: _do_call(self=self, responses_input=responses_input, timeout_s=timeout_s),
