...
def build_openresponses_request(
    *,
    ...
    if not input_items:
        raise ValueError('build_openresponses_request: requires at least one of prompt or non-empty conversation')
    ...
    for item in input_array:
        ...
        if role in ('user', 'assistant', 'system'):
            ...
            if isinstance(content, list):
                text = ''.join(str(block.get('text') or '') if isinstance(block, dict) else '' for block in content)
            else:
                text = str(content)
...
def load_openresponses_dataset(path: str | Path) -> StaticDataset:
    ...
    if p.suffix.lower() == '.jsonl':
        ...
        if isinstance(loaded, dict) and 'samples' in loaded:
            ...
            raise ValueError(
                f'Unexpected dataset shape in {p}: expected a top-level list or {{"samples": [...]}} object'
            )
