...
def render_attack_fragment(r: RedTeamResult) -> str:
    ...
    # Deep-link to the last successful target-agent trace when known; fall back to
    # the thread filter for older reports (hidden when unconfigured or no id).
    trace_url = single_trace_url(r.last_trace_id)
    trace_btn = trace_link_button(
        trace_url or thread_trace_url(r.thread_id),
