...
def _render_agent_comparison_section(section: ReportSection) -> str:
    ...
    if agent_metrics:
        ...
        metric_rows = [
            [m['agent'], str(m['total_attacks']), str(m['vulnerabilities_found']), _bold_bar(m.get('asr', 0.0))]
...
def _render_attack_heatmap_section(section: ReportSection) -> str:
    ...
    legend = '\n*Legend: Values show Attack Success Rate (ASR). **Bold** = ASR >= 40%. `-` = no attacks attempted.*'
    return f'## {section.title}\n\n{table}{legend}'
...
def _render_error_analysis_section(section: ReportSection) -> str:
    ...
    if error_rate > 0.10:
        lines.extend((
            '> [!WARNING]',
            f'> Error rate is {_pct(error_rate)} ({total_errors} errors). This may indicate infrastructure problems or misconfigured targets.',
            '',
        ))

    lines.extend((
        f'**Total errors:** {total_errors} ({_pct(error_rate)} of attacks) across {data.get("error_types_count", len(errors_by_type))} error type(s).',
        '',
    ))
    ...
    if errors_by_type:
        type_rows = [
            ...
            for error_type, count in sorted(errors_by_type.items(), key=operator.itemgetter(1), reverse=True)
        ]
...
def _render_source_distribution_section(section: ReportSection) -> str:
    ...
    table_rows = [[r['source'], str(r['count']), _bar(r['count'] / total if total > 0 else 0.0)] for r in rows]
...
def _render_token_usage_section(section: ReportSection) -> str:
    ...
    if overall:
        ...
        # Flag a lower-bound total: some calls in this run reported no cost. The
        # same label qualifies the input/output split — it comes from the same
        # `Usage` and the same pricing pass, so an unqualified split beside a
        # qualified total would imply the split alone is exact.
        ...
        if input_cost is not None:
            metric_rows.append(['Input Cost', f'{_fmt_cost(input_cost)}{coverage}'])
        if output_cost is not None:
            metric_rows.append(['Output Cost', f'{_fmt_cost(output_cost)}{coverage}'])
...
def _render_severity_definitions_section(section: ReportSection) -> str:
    ...
    table_rows = [[d['level'].capitalize(), d['description'], str(weights.get(d['level'], ''))] for d in definitions]
...
def _render_methodology_section(section: ReportSection) -> str:
    ...
    if untested:
        lines.extend((
            ...
            '',
            '> [!WARNING]',
            f'> {len(untested)} supported categories were not included in this assessment:',
            '',
        ))
        for cat in untested:
