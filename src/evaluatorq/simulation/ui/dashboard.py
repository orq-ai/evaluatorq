...
def _render_turn_quality(sections: list[ReportSection]) -> None:
    ...
    if dist:
        ...
        fig.update_layout(xaxis_title='Turns', yaxis_title='Conversations', margin={'t': 10, 'b': 10}, height=260)
        st.plotly_chart(fig, width='stretch')
...
def _render_dashboard() -> None:
    ...
    tabs = st.tabs(['Overview', 'Breakdown', 'Transcripts', 'Turn quality', 'Tokens', 'Evaluators', 'Judge & errors'])
    with tabs[0]:
