...
def build_scenario_user_context(scenario: Scenario) -> str:
    ...
    if scenario.criteria:
        must_happen = [delimit(c.description) for c in scenario.criteria if c.type == 'must_happen']
        must_not = [delimit(c.description) for c in scenario.criteria if c.type == 'must_not_happen']
...
def generate_datapoint(persona: Persona, scenario: Scenario, first_message: str = '') -> SimulationDatapoint:
