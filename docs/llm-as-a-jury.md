...
- **Settings a classify judge cannot use are named in a warning.** With no prompt and no verdict schema on this path, `system_prompt`, `prompt`, `temperature` and `structured_output` have nothing to act on: `llm_jury()` names the ones you set in a single warning at construction, and on a mixed panel they still apply in full to the prompted judges. The call settings that live on the judge config — `max_tokens`, `reasoning_effort`, `extra_kwargs`, `extra_body` and `api` — are named the same way by `run_judge` per classify call.
