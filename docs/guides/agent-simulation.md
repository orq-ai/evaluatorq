...
`evaluator_names` picks from four built-ins. Names must be unique because evaluator detail and numeric scores are keyed by name. Two read the judge's verdicts; two apply a *policy* you can change with `scoring=`:
...
class AuditHooks(DefaultHooks):
    async def on_evaluator_complete(
        ...
        if score.error is not None:
            print(f"{datapoint_id} {name} dropped: {score.error}")
            return
