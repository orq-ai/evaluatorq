...
| `EVALUATORQ_RECOMMENDATION_FACTUAL_ACCURACY_BELOW` | No | `0.5` | Simulation recommendations: a conversation-average factual-accuracy below this marks the result remediable. Range 0 to 1; out-of-range or non-finite values fall back with a warning. |
| `EVALUATORQ_RECOMMENDATION_HALLUCINATION_RISK_ABOVE` | No | `0.5` | Simulation recommendations: a conversation-average hallucination-risk above this marks the result remediable. Range 0 to 1. |
| `EVALUATORQ_RECOMMENDATION_TONE_APPROPRIATENESS_BELOW` | No | `0.5` | Simulation recommendations: a conversation-average tone-appropriateness below this marks the result remediable. Range 0 to 1. |
| `EVALUATORQ_RECOMMENDATION_MAX_TRANSCRIPT_CHARS` | No | `3000` | Transcript budget for the recommendation prompt. Minimum 100; lower values fall back with a warning. |
| `EVALUATORQ_RECOMMENDATION_MAX_SUGGESTIONS` | No | `3` | Cap on suggestions per analyzed result. Minimum 1. |
