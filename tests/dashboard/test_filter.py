...
class TestFilterOOBSidebar:
    ...
    def test_redteam_oob_csv_link_round_trip_fewer_rows(self, client: TestClient, roots: list[Path]) -> None:
        ...
        assert len(filtered_rows) < len(all_rows), (
            f'Filtered CSV ({len(filtered_rows)} lines) not fewer than unfiltered ({len(all_rows)} lines)'
        )

    def test_sim_filter_post_has_oob_sidebar(self, client: TestClient, roots: list[Path]) -> None:
