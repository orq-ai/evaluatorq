...
class TestPreview:
    def test_preview_without_api_key_shows_error_drawer(
    ...
    def test_preview_renders_diff_and_confirm(
        self, apply_client, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        client, rid, path = apply_client
    ...
    def test_preview_error_becomes_drawer_not_500(
        self, apply_client, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        client, rid, path = apply_client
        monkeypatch.setattr(apply_mod, "_build_clients", lambda: (object(), object(), "m"))
...
class TestConfirm:
    ...
    def test_confirm_updates_agent_and_records_on_report(
        self, apply_client, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        client, rid, path = apply_client
        calls: list[dict] = []
...
class TestUnits:
    def test_dismiss_returns_empty(self, apply_client) -> None:
        ...
        r = client.get("/redteam/apply/dismiss")
        assert r.status_code == 200
        assert r.text == ""
