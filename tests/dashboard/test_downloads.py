...
class TestDownloadSidebarOnReportPage:
    def test_redteam_page_has_html_download_link(self, client: TestClient, roots: list[Path]) -> None:
        rid = report_id(_rt_path(roots))
        r = client.get(f'/r/{rid}')
        assert 'export.html' in r.text

    def test_redteam_page_has_md_download_link(self, client: TestClient, roots: list[Path]) -> None:
        rid = report_id(_rt_path(roots))
        r = client.get(f'/r/{rid}')
        assert 'export.md' in r.text

    def test_redteam_page_has_csv_download_link(self, client: TestClient, roots: list[Path]) -> None:
        rid = report_id(_rt_path(roots))
        r = client.get(f'/r/{rid}')
        assert 'export.csv' in r.text

    def test_redteam_page_has_json_download_link(self, client: TestClient, roots: list[Path]) -> None:
        rid = report_id(_rt_path(roots))
        r = client.get(f'/r/{rid}')
        assert 'export.json' in r.text

    def test_sim_page_has_html_download_link(self, client: TestClient, roots: list[Path]) -> None:
        rid = report_id(_sim_path(roots))
        r = client.get(f'/r/{rid}')
        assert 'export.html' in r.text

    def test_sim_page_has_json_download_link(self, client: TestClient, roots: list[Path]) -> None:
        rid = report_id(_sim_path(roots))
        r = client.get(f'/r/{rid}')
        assert 'export.json' in r.text

    def test_sim_page_has_md_download_link(self, client: TestClient, roots: list[Path]) -> None:
        """C1: sim page must now show a Markdown download link."""
        rid = report_id(_sim_path(roots))
        r = client.get(f'/r/{rid}')
        assert 'export.md' in r.text

    def test_sim_page_has_no_csv_download_link(self, client: TestClient, roots: list[Path]) -> None:
...
class TestOOBSidebarAfterFilterPost:
    ...
    def test_filter_post_response_contains_oob_sidebar(self, client: TestClient, roots: list[Path]) -> None:
    ...
    def test_filter_post_oob_sidebar_has_stable_id(self, client: TestClient, roots: list[Path]) -> None:
    ...
    def test_filter_post_oob_sidebar_csv_carries_filter_param(self, client: TestClient, roots: list[Path]) -> None:
    ...
    def test_filter_post_oob_sidebar_json_carries_filter_param(self, client: TestClient, roots: list[Path]) -> None:
    ...
    def test_filter_round_trip_csv_fewer_rows(self, client: TestClient, roots: list[Path]) -> None:
        ...
        assert len(filtered_lines) < len(all_lines), (
            f'Expected fewer rows in filtered CSV ({len(filtered_lines)}) than full CSV ({len(all_lines)})'
        )

    def test_filter_round_trip_json_fewer_rows(self, client: TestClient, roots: list[Path]) -> None:
        ...
        assert len(filtered_rows) < len(all_rows), (
            f'Expected fewer rows in filtered JSON ({len(filtered_rows)}) than full JSON ({len(all_rows)})'
        )

    def test_unfiltered_post_sidebar_has_no_querystring(self, client: TestClient, roots: list[Path]) -> None:
...
class TestEmptySelectionsExportAll:
    ...
    def test_redteam_csv_empty_selections_returns_all_rows(self, client: TestClient, roots: list[Path]) -> None:
        ...
        assert len(data_lines) == 5, f'Expected 5 lines (1 header + 4 data rows), got {len(data_lines)}: {data_lines}'

    def test_redteam_json_empty_selections_returns_all_rows(self, client: TestClient, roots: list[Path]) -> None:
        """export.json with no query params must return ALL 4 results."""
        rid = report_id(_rt_path(roots))
        r = client.get(f'/r/{rid}/export.json')
        assert r.status_code == 200
        rows = json.loads(r.text)
        assert len(rows) == 4, f'Expected 4 rows (all unfiltered), got {len(rows)}'

    def test_sim_json_empty_selections_returns_all_rows(self, client: TestClient, roots: list[Path]) -> None:
        """export.json with no query params must return ALL 2 sim results."""
        rid = report_id(_sim_path(roots))
        r = client.get(f'/r/{rid}/export.json')
        assert r.status_code == 200
        rows = json.loads(r.text)
        assert len(rows) == 2, f'Expected 2 rows (all unfiltered sim results), got {len(rows)}'
