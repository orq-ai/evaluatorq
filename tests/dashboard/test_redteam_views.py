...
- GET /r/{rid}/redteam/attack  — attack evidence fragment; idx param
...
def _make_result(
    *,
    ...
    return RedTeamResult(
        ...
        response=response,
        evaluation=(UnifiedEvaluationResult(passed=passed, explanation=explanation) if passed is not None else None),
    )
...
class TestBreakdownView:
    ...
    def test_asr_recomputed_known_distribution(self, tmp_path: Path) -> None:
        ...
        html = (
            TestClient(known_app, raise_server_exceptions=True)
            .get(f'/r/{known_rid}/view/breakdown?group_by=category')
            .text
        )
        ...
        assert asi01_row['value'] == 50.0, f'Expected ASI01 ASR=50.0, got {asi01_row["value"]!r}'
    ...
    def test_asr_evaluated_set_denominator_matches_static_report(self, tmp_path: Path) -> None:
        ...
        html = (
            TestClient(denom_app, raise_server_exceptions=True)
            .get(f'/r/{denom_rid}/view/breakdown?group_by=category')
            .text
        )
        assert html, 'Expected non-empty HTML from breakdown'
...
# ---------------------------------------------------------------------------
# 3. Attack fragment endpoint (replaces the deleted conversation viewer)
# ---------------------------------------------------------------------------


class TestAttackFragmentView:
    """GET /r/{rid}/redteam/attack"""

    def test_returns_200(self, client: TestClient, rid: str) -> None:
        _get(client, f'/r/{rid}/redteam/attack?idx=0')

    def test_renders_evaluator_verdict(self, client: TestClient, rid: str) -> None:
        html = _get(client, f'/r/{rid}/redteam/attack?idx=0')
        assert 'Evaluator verdict' in html

    def test_idx_changes_fragment(self, client: TestClient, rid: str) -> None:
        """Different idx values should produce different fragments."""
        html_0 = _get(client, f'/r/{rid}/redteam/attack?idx=0')
        html_1 = _get(client, f'/r/{rid}/redteam/attack?idx=1')
    ...
    def test_hx_get_links_present_for_navigation(self, client: TestClient, rid: str) -> None:
        html = _get(client, f'/r/{rid}/redteam/attack?idx=0')
        assert 'rt-msg' in html or 'Evaluator verdict' in html
...
class TestDisagreementView:
    ...
    def test_page_1_and_page_2_differ_when_enough_disagreements(self, tmp_path: Path) -> None:
    ...
    def test_url_encoded_agent_keys_round_trip(self, tmp_path: Path) -> None:
        ...
        html = sp_client.get(
            f'/r/{sp_rid}/view/disagreement?a={quote("agent b", safe="")}&b={quote("agent&b", safe="")}&page=1'
        ).text
    ...
    def test_no_disagreements_shows_info(self, tmp_path: Path) -> None:
        ...
        html = agree_client.get(f'/r/{agree_rid}/view/disagreement?a=agent-a&b=agent-b&page=1').text
...
class TestMountPoints:
    ...
    def test_report_page_has_htmx_view_routes(self, client: TestClient, rid: str) -> None:
        r = client.get(f'/r/{rid}')
        assert r.status_code == 200
        assert '/view/breakdown' in r.text or '/redteam/attack' in r.text
...
class TestViewRoutesHonorFilter:
    ...
    def test_breakdown_unfiltered_vs_filtered_differ(self, tmp_path: Path) -> None:
        ...
        html_vuln = client.get(f'/r/{rid}/view/breakdown?group_by=category&result=Vulnerable').text
    ...
    def test_breakdown_filter_drops_resistant_category(self, tmp_path: Path) -> None:
        ...
        assert 'LLM01' not in html, 'Expected LLM01 category to be absent when filtering to Vulnerable only'
    ...
    def test_attack_fragment_filtered_changes_result(self, tmp_path: Path) -> None:
        """Filtering to Vulnerable should index into a different (filtered) result set."""
        client, rid = self._filtered_report(tmp_path)
    ...
    def test_disagreement_filtered_reduces_or_changes_set(self, tmp_path: Path) -> None:
        ...
        html_llm = c.get(f'/r/{rid}/view/disagreement?a=agent-a&b=agent-b&page=1&category=LLM01').text
...
class TestPanelContainerFilterWiring:
    ...
    def test_panel_containers_include_filter_form(self, client: TestClient, rid: str) -> None:
    ...
    def test_filter_form_has_stable_id(self, client: TestClient, rid: str) -> None:
        ...
        r = client.get(f'/r/{rid}')
        assert r.status_code == 200
        assert 'id="filter-form"' in r.text, 'Expected id="filter-form" on the filter form element.'
...
class TestFilterPostEmitsHxTrigger:
    ...
    def test_filter_post_returns_hx_trigger_header(self, client: TestClient, rid: str) -> None:
        ...
        assert 'orq:filter-changed' in hx_trigger, (
            f'Expected HX-Trigger: orq:filter-changed in response headers, got: {hx_trigger!r}'
        )

    def test_filter_post_with_selections_still_emits_hx_trigger(self, client: TestClient, rid: str) -> None:
