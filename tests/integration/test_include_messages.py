"""
Integration test for includeMessages handling.

This script:
1. Creates a temporary dataset via Orq SDK
2. Adds a datapoint with top-level messages
3. Runs evaluatorq with include_messages=True
4. Verifies that the messages are included in the job input
...
async def test_include_messages_works_without_conflict(orq_client: Orq, test_dataset: str) -> None:
