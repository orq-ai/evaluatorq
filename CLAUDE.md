...
- **When you (Claude) open a PR, label it at creation time.** Pass `--label` to `gh pr create` using the same mapping, derived from the PR's conventional-commit title prefix: `feat→feature`, `fix`/`perf`/`revert`→`fix`, `docs→docs`, `chore`/`build`/`test`/`style`→`chore`, `ci→ci`, `refactor→refactor`. E.g. a PR titled `fix: …` → `gh pr create --label fix …`. If the title isn't conventional, pick the closest label or leave it unlabeled (it lands in *Other*).
  - **The autolabeler is a backstop, not the primary path.** If you forget, `label-pr.yml` applies the same label on PR open/edit for those mapped prefixes — so the mapped items get categorized either way. But always label at creation: it's earlier (visible the moment the PR opens, before any CI race) and explicit. Prefer early and verbose over relying on the pipeline to catch it.
