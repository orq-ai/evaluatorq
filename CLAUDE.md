...
- **Accidental majors are refused.** A computed `major` bump is skipped unless you re-run via **workflow_dispatch** with `force_level=major`. Use `force_level=minor`/`patch` to override the computed level (e.g. to ship breaking changes as a minor deliberately). The refusal is a `::warning::` and the job still exits **green** — a blocked release looks identical to a no-op release, so a stuck pipeline is invisible unless you read the logs. The `!` commits stay in range until someone releases, so the block is permanent, not transient.
