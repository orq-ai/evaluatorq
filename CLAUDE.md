...
| Prompt caching on a replayed conversation | `common.prompt_cache` (`apply_cache_breakpoints` for chat, `mark_responses_input` for Responses — both take `volatile_tail=N`), gated on `client_routes_through_orq` | hand-placed `cache_control`, or a `prompt_cache_key` (OpenAI caches automatically; it needs none) |
